...
判断两个链表是否相交，需要定义两个指针 `nodeA` 和 `nodeB`，假设两个链表长度分别为 `m` 和 `n`，则 `nodeA` 先遍历 `headA` 后遍历 `headB` 和 `nodeB` 先遍历 `headB` 后遍历 `headA` 的步数是一样的（`m + n = n + m`），因此当两者步数一样时，即可得知相交的位置。

- `nodeA` 先遍历 `headA`， `nodeB` 先遍历 `headB`。
- 当 `nodeA` 遍历结束后，`nodeA` 设置为 `headB` 的根结点。
- 当 `nodeB` 遍历结束后，`nodeB` 设置为 `headA` 的根结点。
- 每一次都比较 `nodeA` 和 `nodeB` 是否一样，如果一样，则为相交节点。
- 注意点：如果两个节点最后为 `null`，则证明不相交。
...
var getIntersectionNode = function (headA, headB) {
    let nodeA = headA, nodeB = headB, flagA = false, flagB = false;
    while (nodeA !== nodeB) {
        nodeA = nodeA.next;
        nodeB = nodeB.next;
        if (nodeA === null && !flagA) { nodeA = headB; flagA = true; }
        if (nodeB === null && !flagB) { nodeB = headA; flagB = true }
    }
    return nodeA;
};
```
